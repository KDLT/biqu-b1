...
motorMountOffset=[15.3, 4.56, riseHeight+sleeveThickness]; // Trial 5 (sipat) 15.3 na kasi nilakihan ko by 0.5 ang motor_sideLength para lumuwag

module motorHousingAndBase(l=motor_sideLength, h=facePlateHeight, mo=motorMountOffset, t=sleeveThickness, sh=sleeveHeight, rh=riseHeight, c=chamferSize, shaft=false, shaftD=2, shaftH=20) {
...
capDiameter = 7; // versions = [6, 6.5, 7]
dCoupler = 26.4; // enlarged size for the ptfe coupler slot
...
module motorBracketAssembly() {
    motorHousingAndBase(shaft=false, shaftD=7.4, shaftH=filamentFaceplateYOffset);
    difference() {
        topPlate(dc=capDiameter, db=dCoupler, rh=totalHeight, filament=false, filamentD=1.75); // this is the main plate plus the riser
        vertHoles();
    };
};

motorBracketAssembly();
